...
public class SwipeManagementTest {
	 ...
	 * 
	 * @Order(value = 1) public void getFacilities() { List<Facility> lf = new
	 * ArrayList<Facility>(); List<Facility> lst = service.getFacilities();
	...
	public void saveOrUpdate() {
		...
		Location lo = new Location(101, "HYD");
		List<Location> lst = new ArrayList<Location>();
		lst.add(lo);
		Facility facility = new Facility(2,"Mindspace",lst);
		System.out.println("FacilityLocation is"+facility.getFacilityid()+" "+facility.getFacilityname()+" "+lst.toString());
		FacilityLocation facility1 = new FacilityLocation(facility); 
		System.out.println("FacilityLocation is"+facility1.getFacility().toString());
		when(smRepository.save(facility1.getFacility())).thenReturn(facility1.getFacility());
		Facility facility2 = service.saveOrUpdate(facility1);
        assertEquals(facility2,facility1.getFacility());
		/*
